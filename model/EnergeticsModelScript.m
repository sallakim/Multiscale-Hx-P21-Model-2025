% EnergeticsModelScript
 function out = EnergeticsModelScript(data)
% INPUTS (arguments for cellular energetics model)
...
%% Unpack data 
TAN = data.TAN_sham;
CRtot = data.CRtot_sham;
TEP = data.TEP_sham; 
Ox_capacity = data.Ox_capacity_sham; 
x_ATPase = data.x_ATPase_sham; 
...
% Substrate transport: Passive permeation of ATP, ADP, AMP and Pi between
% the external buffer and IM space (mitochondrial energetics model)
% im:
ATP_i = ATP_c;
ADP_i = ADP_c;
AMP_i = AMP_c;
...
%% Make sure all the variables except the membrane potential cannot become negative (inside of membrane is negative due to ions)
%% Solve Model
varlist = 1:28;
...
% Fluxes (reaction and transpport fluxes from energetics model)
J(3)=J(3);
...
out = [MgATP_cytoplasm*1000 MgADP_cytoplasm*1000 fPi_cytoplasm*1000 J_ATPase_tissue MVO2_tissue dGrATPase PCrATP ATP_cyto ADP_cyto Pi_cyto*1000];
